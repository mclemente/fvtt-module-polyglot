class PolyGlot {
    ...
    static get defaultLanguage() {
            const defaultLang = game.settings.get("poliglota", "defaultLanguage");
            if (defaultLang) {
 ...
 async renderChatLog(chatlog, html, data) {
            await this.setCustomLanguages(game.settings.get("poliglota", "customLanguages"))
    ...
    updateChatMessagesDelayed() {
            ...
            for (let i = messages.length - 1; i >= 0; i--) {
                    ...
                    if (message.data.type == CONST.CHAT_MESSAGE_TYPES.IC || this._isMessageTypeOOC(message.data.type)) {
                            let lang = message.getFlag("poliglota", "language") || ""
                            let unknown = !this.known_languages.has(lang);
                            if (game.user.isGM && !game.settings.get("poliglota", "runifyGM")) {
                                    // Update globe color
    ...
    renderChatMessage(message, html, data) {
            ...
            const lang = message.getFlag("poliglota", "language") || ""
            if (!lang) return;
            ...
            if (!message.polyglot_force && message.polyglot_unknown) {
                    ...
                    let new_content = this.scrambleString(message.data.content, game.settings.get("poliglota",'useUniqueSalt') ? message.data._id : lang)
    ...
    preCreateChatMessage(data, options, userId) {
            if (data.type == CONST.CHAT_MESSAGE_TYPES.IC || (this.allowOOC && this._isMessageTypeOOC(data.type) && game.user.isGM)) {
                    ...
                    if (lang != "")
                            mergeObject(data, { "flags.poliglota.language": lang });
            }
    }
    ...
    async loadLanguages(setting) {
            const response = await fetch(`modules/poliglota/settings/${setting}.json`);
            if (response.ok) {
    ...
    setup() {
            ...
            // custom languages
            game.settings.register("poliglota", "customLanguages", {
            ...
            game.settings.register("poliglota", "defaultLanguage", {
            ...
            game.settings.register("poliglota", "runifyGM", {
            ...
            game.settings.register("poliglota", "useUniqueSalt", {
            ...
            game.settings.register("poliglota", "exportFonts", {
            ...
            ChatBubbles.prototype._getMessageDimensions = (message) => {
             ...
             // allow OOC talking
            game.settings.register("poliglota", "allowOOC", {
            ...
            this.allowOOC = game.settings.get("poliglota","allowOOC");
    }
    ready() {
    ...
    updateConfigFonts() {
            ...
            if (game.settings.get("poliglota", "exportFonts")) {
                    ...
                    toggleButton.click(ev => {
                            ...
                            if (runes) {
                                    ...
                                    for (let span of spans.toArray()) {
                                            ...
                                            span.textContent = this.scrambleString(span.textContent, game.settings.get("poliglota",'useUniqueSalt') ? journalSheet._id : lang)
            ...
            if (message.data.type == CONST.CHAT_MESSAGE_TYPES.IC) {
                    ...
                    if (lang != "") {
                            ...
                            if (!message.polyglot_force && message.polyglot_unknown) {
                                    ...
                                    const new_content = this.scrambleString(message.data.content, game.settings.get("poliglota",'useUniqueSalt') ? message._id : lang)
                                    content.text(new_content)
    ...
    vinoChatRender (chatDisplayData) {
            ...
            if (lang != "") {
                    ...
                    if (game.user.isGM && !game.settings.get("poliglota", "runifyGM"))
                            message.polyglot_unknown = false;
                    if (!message.polyglot_force && message.polyglot_unknown) {
                            const new_content = this.scrambleString(chatDisplayData.text, game.settings.get("poliglota",'useUniqueSalt') ? message._id : lang)
